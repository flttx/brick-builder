...
interface BrowserSceneSample {
  samples: FrameSample[];
  metrics: { drawCalls: number; batchCount: number; chunkCount: number; instances: number; triangles: number; dpr: number; quality: string };
...
function renderMarkdown(report: { browser: string; os: string; metrics: BrowserBenchmarkResult[] }): string { return [`# Browser Performance Report`, ``, `- Browser: ${report.browser}`, `- OS: ${report.os}`, `- Renderer benchmark: real browser, fixed 1440×900 viewport, Balanced, 3s warmup + 10s samples`, `- Real-device status: not established by this headless/host run`, ``, `| Size | Layout | FPS P50 | FPS P05 | Frame P95 | Draw Calls | Instances | DPR |`, `| ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: |`, ...report.metrics.map((item) => `| ${item.size} | ${item.layout} | ${item.fps.p50.toFixed(2)} | ${item.fps.p05.toFixed(2)} | ${item.frameMs.p95.toFixed(2)} | ${item.drawCalls} | ${item.instances} | ${item.dpr.toFixed(2)} |`)].join("\n") + "\n"; }
