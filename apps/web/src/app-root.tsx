...
const ProjectCard = ({ project, localEntry, renaming, name, renameError, onOpen, onStartRename, onNameChange, onSaveRename, onCancelRename, onDuplicate, onDelete, onTogglePin }: ProjectCardProps): ReactElement => {
  const [thumbnailFailed, setThumbnailFailed] = useState(false);
  useEffect(() => setThumbnailFailed(false), [project.thumbnailUrl]);
  const displayName = localizeProjectName(project.name);
  const pinLabel = localEntry?.isPinned === true ? messages.builds.unpin : messages.builds.pin;
  const showThumbnailPlaceholder = project.thumbnailUrl === undefined || thumbnailFailed;

  return <article className="project-card">
    <div className="project-thumbnail" aria-label={messages.builds.thumbnail(displayName)}>{showThumbnailPlaceholder ? <div className="thumbnail-placeholder" aria-hidden="true"><span /><span /><span /></div> : <img src={project.thumbnailUrl} alt="" onError={() => setThumbnailFailed(true)} />}</div>
    <div className="project-card-body">
      <div className="project-card-heading">
        {renaming ? <div className="rename-editor"><div className="rename-row"><input value={name} onChange={(event) => onNameChange(event.target.value)} aria-label={messages.builds.projectName} placeholder={messages.builds.projectNamePlaceholder} maxLength={80} autoFocus /><button type="button" onClick={onSaveRename}>{messages.common.save}</button><button type="button" onClick={onCancelRename}>{messages.common.cancel}</button></div>{renameError !== undefined && <p className="inline-error" role="alert">{renameError}</p>}</div> : <h3>{displayName}</h3>}
        {localEntry?.isPinned === true && <span className="pin-badge">{messages.builds.offlineReady}</span>}
      </div>
      <div className="project-card-meta">
        <p className="project-card-summary">{messages.builds.summary(project.brickCount, formatDateTimeZhCN(project.updatedAt))}</p>
        {localEntry?.offlineReady === true && <span className="offline-ready-badge">{messages.builds.offlineReady}</span>}
      </div>
      <div className="project-card-actions">
        <button className="primary-action" type="button" onClick={onOpen}>{messages.builds.open}</button>
        <div className="project-card-secondary-actions">
          <button type="button" onClick={onTogglePin}>{pinLabel}</button>
          <button type="button" onClick={onStartRename}>{messages.common.rename}</button>
          <button type="button" onClick={onDuplicate}>{messages.common.duplicate}</button>
          <button type="button" onClick={onDelete}>{messages.common.delete}</button>
        </div>
      </div>
    </div>
  </article>;
