...
import { IndexedDbCachedProjectStore, IndexedDbLocalDraftStore, IndexedDbLocalProjectIndexStore, RecoveryManager, HttpProjectRepository, localProjectIndexFromMetadata, type LocalProjectIndexEntry, type LocalProjectDraft, type ProjectDetail, type ProjectMetadata, type ProjectRepository } from "../../../packages/project-persistence/index.js";
...
export const AppRoot = (): ReactElement => {
  ...
  const loadProjects = useCallback(async (userId = session?.userId): Promise<void> => {
    ...
    try {
      ...
      await Promise.all(cloudProjects.map(async (project) => indexStore.upsert(localProjectIndexFromMetadata(project, userId, (await indexStore.get(userId, project.id)) ?? undefined))));
    } catch {
      setProjects((await indexStore.list(userId)).map(metadataFromLocalIndex));
      setApiOnline(false);
    }
    setLocalIndex(await indexStore.list(userId));
  ...
  const openProject = useCallback(async (projectId: string, userId = session?.userId): Promise<void> => {
    ...
    try {
      ...
      const [draft, cached] = await Promise.all([draftStore.get(userId ?? "", projectId), cachedStore.get(userId ?? "", projectId)]);
      if (userId !== undefined) await cachedStore.put({ userId, projectId, serverRevision: cloud.serverRevision, snapshot: cloud.snapshot, cachedAt: Date.now() });
      if (userId !== undefined) {
  ...
  useEffect(() => {
    ...
    void repository.getSession().then(async (current) => {
      ...
      setApiOnline(true);
      if (current === null) {
        const offlineUserId = window.localStorage.getItem("brick-builder-active-user");
        if (offlineUserId !== null && routeProjectId !== null) {
          const offlineProject = await recoverOfflineProject(offlineUserId, routeProjectId, draftStore, cachedStore);
          if (offlineProject !== null) { setSession({ userId: offlineUserId, email: messages.auth.offlineMode }); setEditorProject(offlineProject); setApiOnline(false); setView("editor"); return; }
        }
        setView("auth"); return;
      }
      setSession(current);
    ...
    }).catch(async () => { if (!active) return; const offlineUserId = window.localStorage.getItem("brick-builder-active-user"); if (offlineUserId !== null) { try { const offlineEntries = await indexStore.list(offlineUserId); setLocalIndex(offlineEntries); if (routeProjectId !== null) { const offlineProject = await recoverOfflineProject(offlineUserId, routeProjectId, draftStore, cachedStore); if (offlineProject !== null) { setSession({ userId: offlineUserId, email: messages.auth.offlineMode }); setEditorProject(offlineProject); setApiOnline(false); setView("editor"); return; } } if (offlineEntries.length > 0) { setSession({ userId: offlineUserId, email: messages.auth.offlineMode }); setProjects(offlineEntries.map(metadataFromLocalIndex)); setApiOnline(false); setView("builds"); return; } } catch { setAppError(messages.errors.LOCAL_STORAGE_UNAVAILABLE); } } setApiOnline(false); setView("local"); });
    return () => { active = false; };
  }, [draftStore, cachedStore, indexStore, loadProjects, openProject, repository, routeProjectId]);

  const handleAuthenticated = useCallback(async (current: Session): Promise<void> => { loggingOutRef.current = false; window.localStorage.setItem("brick-builder-active-user", current.userId); setSession(current); setApiOnline(true); await loadProjects(); go("builds", "/my-builds"); }, [go, loadProjects]);
...
const MyBuilds = ({ projects, localIndex, email, error, onCreate, onOpen, onRename, onDelete, onTogglePin, onLogout }: MyBuildsProps): ReactElement => {
  ...
  return <main className="builds-shell"><header className="builds-header"><div><p className="eyebrow">{messages.brand}</p><h1>{messages.builds.title}</h1><p className="builds-subtitle">{messages.builds.subtitle}</p></div><div className="builds-account"><span>{email}</span><button type="button" onClick={onLogout}>{messages.builds.logout}</button></div></header>{error !== undefined && <p className="inline-error builds-error" role="alert">{error}</p>}<section className="builds-content" aria-labelledby="builds-list-title"><div className="builds-toolbar"><h2 id="builds-list-title">{messages.builds.recentProjects}</h2><button className="primary-action" type="button" onClick={onCreate}>{messages.builds.newBuild}</button></div>{projects.length === 0 ? <div className="empty-builds"><h2>{messages.builds.emptyTitle}</h2><p>{messages.builds.emptyDescription}</p><button className="primary-action" type="button" onClick={onCreate}>{messages.builds.startBuilding}</button></div> : <div className="project-list">{projects.map((project) => <ProjectCard key={project.id} project={project} localEntry={localIndex.find((entry) => entry.projectId === project.id)} renaming={renaming === project.id} name={renaming === project.id ? name : localizeProjectName(project.name)} {...(renameError === undefined ? {} : { renameError })} onOpen={() => onOpen(project.id)} onStartRename={() => { setRenaming(project.id); setName(localizeProjectName(project.name)); setRenameError(undefined); }} onNameChange={(value) => { setName(value); setRenameError(undefined); }} onSaveRename={() => saveRename(project.id)} onCancelRename={() => { setRenaming(undefined); setRenameError(undefined); }} onDelete={() => setDeleteProject(project)} onTogglePin={() => onTogglePin(project.id)} />)}</div>}</section>{deleteProject !== undefined && <DeleteProjectDialog project={deleteProject} onCancel={() => setDeleteProject(undefined)} onConfirm={() => { onDelete(deleteProject.id); setDeleteProject(undefined); }} />}</main>;
};

interface ProjectCardProps { project: ProjectMetadata; localEntry: LocalProjectIndexEntry | undefined; renaming: boolean; name: string; renameError?: string; onOpen: () => void; onStartRename: () => void; onNameChange: (name: string) => void; onSaveRename: () => void; onCancelRename: () => void; onDelete: () => void; onTogglePin: () => void; }
const ProjectCard = ({ project, localEntry, renaming, name, renameError, onOpen, onStartRename, onNameChange, onSaveRename, onCancelRename, onDelete, onTogglePin }: ProjectCardProps): ReactElement => { const displayName = localizeProjectName(project.name); const pinLabel = localEntry?.isPinned === true ? messages.builds.unpin : messages.builds.pin; return <article className="project-card"><div className="project-thumbnail" aria-label={messages.builds.thumbnail(displayName)}>{project.thumbnailUrl === undefined ? <span aria-hidden="true">BB</span> : <img src={project.thumbnailUrl} alt="" />}</div><div className="project-card-body">{renaming ? <div className="rename-editor"><div className="rename-row"><input value={name} onChange={(event) => onNameChange(event.target.value)} aria-label={messages.builds.projectName} placeholder={messages.builds.projectNamePlaceholder} maxLength={80} autoFocus /><button type="button" onClick={onSaveRename}>{messages.common.save}</button><button type="button" onClick={onCancelRename}>{messages.common.cancel}</button></div>{renameError !== undefined && <p className="inline-error" role="alert">{renameError}</p>}</div> : <h3>{displayName}</h3>}<p>{messages.builds.summary(project.brickCount, formatDateTimeZhCN(project.updatedAt))}</p>{localEntry?.offlineReady === true && <span className="offline-ready-badge">{messages.builds.offlineReady}</span>}<div className="project-card-actions"><button className="primary-action" type="button" onClick={onOpen}>{messages.builds.open}</button><button type="button" onClick={onTogglePin}>{pinLabel}</button><button type="button" onClick={onStartRename}>{messages.common.rename}</button><button type="button" onClick={onDelete}>{messages.common.delete}</button></div></div></article>; };
...
const recoverOfflineProject = async (userId: string, projectId: string, draftStore: IndexedDbLocalDraftStore, cachedStore: IndexedDbCachedProjectStore): Promise<EditorProject | null> => { const [draft, cached] = await Promise.all([draftStore.get(userId, projectId), cachedStore.get(userId, projectId)]); const snapshot = draft?.snapshot ?? cached?.snapshot; const revision = draft?.baseServerRevision ?? cached?.serverRevision; if (snapshot === undefined || revision === undefined) return null; return { id: projectId, name: messages.builds.offlineDraft, brickCount: snapshot.bricks.length, currentRevision: revision, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), snapshot, serverRevision: revision, recoveryNotice: draft === undefined ? messages.editor.offlineCachedNotice : messages.editor.draftRecoveredNotice, ...(draft === undefined ? {} : { recoveredDraft: true }) }; };
