...
export interface DevToolsMetrics {
  frame: { fps: number; frameMs: number; drawCalls: number; instanceCount: number; visibleInstanceCount: number; triangles: number };
...
export const DevToolsShell = (props: DevToolsShellProps): ReactElement | null => {
  ...
  return (
    <aside className="devtools-shell" aria-label={messages.devtools.label}>
      ...
      {props.tab === "performance" && <div className="devtools-panel" role="tabpanel"><p>{props.performance}</p><Readout label="FPS" value={formatNumber(props.metrics.frame.fps, 1)} /><Readout label="Draw calls" value={String(props.metrics.frame.drawCalls)} /><Readout label="Instances" value={String(props.metrics.frame.instanceCount)} /><Readout label="Visible" value={String(props.metrics.frame.visibleInstanceCount)} /></div>}
